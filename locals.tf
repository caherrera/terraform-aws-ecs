locals {
  ecs_client_prefix         = coalesce(var.ecs_client_prefix, var.host)
  ...
  ecs_log_group_name        = coalescelist(var.ecs_log_group_name, "${local.ecs_client_prefix}/${local.container_name}",)
